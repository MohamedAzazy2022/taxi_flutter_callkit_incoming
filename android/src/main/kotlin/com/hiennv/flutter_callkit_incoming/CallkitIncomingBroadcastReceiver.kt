...
class CallkitIncomingBroadcastReceiver : BroadcastReceiver() {
    ...
    private fun saveTaxiPendingAction(context: Context, action: String, data: Bundle?) {
        try {
            val callId = getTaxiExtra(data, "callId")
            if (callId.isBlank()) return

            val prefs = context.getSharedPreferences("taxi_callkit_pending_actions", Context.MODE_PRIVATE)
            prefs.edit()
                .putString("pending_action", action)
                .putString("pending_call_id", callId)
                .putString("pending_channel_name", getTaxiExtra(data, "channelName"))
                .putString("pending_caller_uid", getTaxiExtra(data, "callerUid"))
                .putString("pending_receiver_uid", getTaxiExtra(data, "receiverUid"))
                .apply()
        } catch (_: Exception) {
        }
    }
